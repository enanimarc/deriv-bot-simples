...
<body>
    ...
    <script>
        ...
        // ============================================
        // ESTADO DO BOT (com informações da conta)
        // ============================================
        let ws = null;
        ...
        // ============================================
        // FUNÇÃO PARA COMPRA REAL NA API
        // ============================================
            ...
            try {
                ...
                ws.onmessage = (event) => {
                    ...
                    if(data.msg_type === 'balance' && data.balance) {
                        ...
                        document.getElementById('accountCurrency').innerHTML = data.balance.currency || 'USD';
                        
                        // Log opcional do saldo
                        if (Math.abs(data.balance.balance - botState.lastBalance) > 0.01) {
                            addLog(`💰 Saldo: ${data.balance.currency} ${data.balance.balance.toFixed(2)}`, 'info');
                            botState.lastBalance = data.balance.balance;
                        }
                    }
                    ...
                    if(data.msg_type === 'tick' && data.tick) {
                        ...
                        // Solicitar saldo a cada tick (opcional, pois já está inscrito)
                        // requestBalance();
                    }
                    
                    // ========================================
                    // PROPOSTA RECEBIDA (PRIMEIRO PASSO DA COMPRA)
                    // ========================================
                    if(data.msg_type === 'proposal' && data.proposal) {
                        ...
                        addLog(`📊 Proposta: Stake $${stake.toFixed(2)} | Payout $${payout.toFixed(2)} | Retorno ${return_pct}%`, 'success');
                        
                        // Executar a compra automaticamente
                        ws.send(JSON.stringify({
                            buy: proposal.id,
                            price: stake
                        }));
                    ...
                    if(data.msg_type === 'buy' && data.buy) {
                        ...
                        addLog(`✅ CONTRATO COMPRADO! ID: ${buy.contract_id} | Preço: $${botState.purchasePrice.toFixed(2)}`, 'success');
                    ...
                    if(data.msg_type === 'proposal_open_contract' && data.proposal_open_contract) {
                        ...
                        if (profit > 0) {
                            ...
                            // IMPORTANTE: Manter currentTradeDigit para comprar no próximo tick
                            botState.inPosition = false;
                            botState.entryTriggered = false;
        ...
        // ============================================
        // ESTRATÉGIA PRINCIPAL - MARTINGALE COM API
        // ============================================
        function executeStrategy(lastDigit) {
            ...
            // PASSO 2: Encontrar dígito com 0% (primeira entrada)
            ...
            // PASSO 3: Aguardar 8% (primeira entrada)
